...
GLfloat xpos = 3.9f;
GLfloat ypos = 3.4f;			// Значения для пятого ребуса (1.5, 0.5)
GLfloat hold;					// Temporarily Holds A Floating Point Value
GLfloat zoom = 1.0f;
GLfloat xyzoom = 1;
GLuint  memlist;				// Память для списка отображения


#define N	5					// Количество объектов сцены

SCENE_ELEMENT sc[N];			// Сцена из трех элементов
int neari = -1;					// Индекс активного элемента сцены
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	// Тут надо выводить список запомненный
	for (int i = 0; i < N; i++) {
	...
	switch (uMsg)									// Check For Windows Messages
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			if (wParam & MK_LBUTTON && initXY)
			{
				...
				// Передвинуть позицию активного объекта
				if (neari >= 0 && neari < N)
				{
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow(_T("Linfuong & NeHe's Waving Texture Tutorial"),1366,768,32,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			else									// Not Time To Quit, Update Screen
			{
                ...
                {
                     zoom/=1.002f;              // Уменьшаем
					 xyzoom /= 1.002f;
                }

                if (keys[VK_NEXT])              // Page Down нажата?
                {
                     zoom*=1.002f;             // Увеличиваем
					 xyzoom *= 1.002f;
                }
			}
